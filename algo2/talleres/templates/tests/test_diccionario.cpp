...
TEST(diccionario, dicc_string_bool) {
    ...
    ASSERT_FALSE(d.def("Code"));
    ASSERT_FALSE(d.def("Titus"));
    ASSERT_FALSE(d.def("Tito"));
    d.definir("Titus", false);
    ASSERT_TRUE(d.def("Titus"));
    ASSERT_FALSE(d.def("Tito"));
    ASSERT_FALSE(d.def("Code"));
    ASSERT_EQ(d.obtener("Titus"), false);
    d.definir("Tito", true);
    ASSERT_TRUE(d.def("Titus"));
    ASSERT_TRUE(d.def("Tito"));
    ASSERT_FALSE(d.def("Code"));
    ASSERT_EQ(d.obtener("Titus"), false);
    ASSERT_EQ(d.obtener("Tito"), true);
    d.definir("Tito", false);
    ASSERT_TRUE(d.def("Tito"));
    ASSERT_TRUE(d.def("Titus"));
    ASSERT_FALSE(d.def("Code"));
    ASSERT_EQ(d.obtener("Titus"), false);
    ASSERT_EQ(d.obtener("Tito"), false);
}

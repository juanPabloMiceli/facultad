...
void insert(int i, ListaAtomica<int> *l){
    l->insertar(i);
}

LT_BEGIN_TEST(TestsEjercicio1, InsertarMultiThreads)
    const int N = 10000;
    std::vector<std::thread> threads(N);
    //Insertamos del 1 al 9999.
    for(int i = 0; i < N; i++){
        threads[i] = std::thread(insert, i, &l);
    }
    ...
    //Hay N elementos?
    LT_CHECK_EQ(l.longitud(), N);

    //Esta cada elemento exactamente una vez?
    std::vector<int> numeros(N, 0);
    for(int i = 0; i < N; i++){
        numeros[l[i]]++;
    }
    for(int i = 0; i < N; i++){
        LT_CHECK_EQ(numeros[l[i]], 1);
    }

LT_END_TEST(InsertarMultiThreads)

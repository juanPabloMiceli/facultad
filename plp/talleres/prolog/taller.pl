...
alcanzable(a(I, F, T), S) :- estados(a(I, F, T), EstadosDeA), length(EstadosDeA, M), N is M+1,
...
reconoce(A, P) :- ground(P), length(P, M), N is M+1, inicialDe(A, I), finalesDe(A, F), member(SF, F), caminoDeLongitud(A, N, _, P, I, SF).
reconoce(A, P) :- not(ground(P)), not(hayCiclo(A)), desde(0, N), inicialDe(A, I), finalesDe(A, F), member(SF, F), caminoDeLongitud(A, N, _, P, I, SF).
reconoce(A, P) :- not(ground(P)), hayCiclo(A), desde(0, N), inicialDe(A, I), finalesDe(A, F), member(SF, F), caminoDeLongitud(A, N, _, P, I, SF).

% 10) PalabraMásCorta(+Automata, ?Palabra)
palabraMasCorta(A, Palabras) :- primerPalabra(A, P), length(P, N), reconoce(A, Palabras), length(Palabras, N).

% primerPalabra(+Automata, -Palabra)
primerPalabra(A, P) :- reconoce(A, P), !.
%-----------------
%----- Tests -----
%-----------------
